...
export async function createServer(opts: ServerOptions): Promise<McpServer> {
  ...
  server.tool(
    ...
    { readOnlyHint: true },
    async ({ workflow_name, min_pass_rate, lookback_runs }) => {
      try {
  ...
  server.prompt(
    "write-test-case",
    {
      fixture_description: z
        .string()
        .describe(
          "Describe what the fixture should test, e.g. 'search for a term and verify results contain it'",
        ),
      tool_names: z
        .string()
        .optional()
