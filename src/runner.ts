...
import {
  ...
  evaluateAllAssertionsAsync,
  type AssertionResult,
} from "./assertions.js";
import type { Fixture, FixtureStep } from "./fixture.js";
...
// ── Step output context for piping ───────────────────────────────────────────

type StepOutputContext = Map<string, string>;

/**
 * Resolve `{{steps.<id>.output}}` placeholders in input values.
 */
function resolveInputPlaceholders(
  ...
  for (const [key, value] of Object.entries(input)) {
    if (typeof value === "string") {
      resolved[key] = value.replace(/\{\{steps\.([^}]+)\.output\}\}/g, (_match, stepId: string) => {
...
function simulateStep(step: FixtureStep, context: StepOutputContext): StepRunResult {
  const start = Date.now();
...
export async function runCase(
  ...
  const context: StepOutputContext = new Map();
  ...
  const status: "pass" | "fail" | "error" = allPassed
    ? "pass"
...
export async function runSuite(
  ...
  const caseResults: CaseRunResult[] = new Array(fixtures.length);
  let passed = 0;
  let failed = 0;
