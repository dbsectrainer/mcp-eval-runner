...
function assertionRows(assertions: AssertionResult[]): string {
...
export function generateHtmlReportTool(runId: string, db: EvalDb): string {
  ...
  const caseRows = cases
    .map((c) => {
      ...
      let stepsAssertions: AssertionResult[][] = [];
      try {
        if (c.assertions_json) {
          stepsAssertions = JSON.parse(c.assertions_json) as AssertionResult[][];
        }
      } catch {
        // ignore parse errors
      }
