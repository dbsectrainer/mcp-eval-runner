...
import type { EvalDb, CaseResultRecord } from "../db.js";

/**
 * regression_report — compare current state to the last run; returns what changed.
 */
export async function regressionReportTool(fixturesDir: string, db: EvalDb): Promise<string> {
 ...
 */
export async function compareResultsTool(
  runIdA: string,
  runIdB: string,
  db: EvalDb,
): Promise<string> {
...
function diffRuns(
  ...
  const prevMap = new Map<string, CaseResultRecord>(prevCases.map((c) => [c.case_name, c]));
  const latestMap = new Map<string, CaseResultRecord>(latestCases.map((c) => [c.case_name, c]));
